...
namespace vision_filter {
  ...
  public:
    ...
    virtual void execute(cv::Mat &image) {
      ...
      if( horizontalBar.size() >= 2)
      {
        ...
        for (int ref_idx = 0, size_ref = horizontalBar.size() ; ref_idx < size_ref; ref_idx++)
        {
          if( IsSplitBar(horizontalBar[0], horizontalBar[1]) )
          {
            ...
            horizontalBar[0] = new ObjectFullData(originalImage, image, tmp);
          }
        }
      }
      ...
      if (horizontalBar.size() != 0) {
        ...
        fence.setTarget(center.x, center.y,
        ...
        if (_search_only_bottom()) {
          ...
          if (_debug_contour()) {
            cv::circle(_output_image, center, 5, CV_RGB(0, 255, 255),
                       20);
          }
          setCameraOffset(&center, image.rows, image.cols);
  ...
  private:
    ...
    {
      float ratio_diff = abs(comp->GetRatio() - ref->GetRatio())/ref->GetRatio();
      float y_diff = abs(comp->GetCenter().y - ref->GetCenter().y)/ref->GetCenter().y;
