...
namespace vision_filter {
  ...
  public:
    ...
    : Filter(globalParams),
      ...
      _difference_from_target_ratio("Diff_from_ratio_target", 0.10f, 0.0f,1.0f, parameters_),
      _targeted_angle("angle_target", 0.0f, 0.0f, 90.0f, parameters_),
      _difference_from_target_angle("Diff_from_angle_target", 30.0f, 0.0f,90.0f, parameters_),
      _contour_retreval("Contour_retreval", 0, 0, 4, parameters_,"0=All, 1=Out, 2=Inner, 3=InnerMost, 4=OutNoChild"),
      ...
      _vote_less_difference_from_targeted_ratio("Vote_less_diff_from_target_ratio", false, parameters_),
    ...
    virtual void execute(cv::Mat &image) {
      if (_enable()) {
        ...
        for (int i = 0, size = contours.size(); i < size; i++) {
          ...
          float percent_filled = calculatePourcentFilled(image, object->GetUprightRect());
          ...
          if (_look_for_rectangle() && !IsRectangle(contours[i], 10)) {
            //if (_look_for_rectangle() && !IsSquare(contours[i], _min_area(), 80.0f, 0.0f, 100.0f)) {
            continue;
          }
        ...
        if(objVec.size() > 1)
        {
          if(_vote_most_centered())
          {
            std::sort(objVec.begin(), objVec.end(),
                      ...
                      -> bool { return a->GetDistanceFromCenter()
                          < b->GetDistanceFromCenter(); });
            objVec[0]->IncrementVote();
            cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, 12),6, CV_RGB(255, 0, 0),-1 );
          }
          ...
          if( _vote_length() )
          {
            ...
            objVec[0]->IncrementVote();
            cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, 6),6, CV_RGB(0, 0, 255),-1 );
          }
          ...
          if( _vote_most_upright() )
          {
            std::sort(objVec.begin(), objVec.end(),
                      [](ObjectFullData::Ptr a, ObjectFullData::Ptr b)
                      -> bool {
              return fabs(a->GetRotatedRect().angle) < fabs(b->GetRotatedRect().angle);
            });
            objVec[0]->IncrementVote();
            cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, 0),6, CV_RGB(255, 0, 255),-1 );
          }
          ...
          if( _vote_less_difference_from_targeted_ratio() )
          {
            std::sort(objVec.begin(), objVec.end(),
                      ...
                      -> bool {
              return fabs(a->GetRatio()-this->_targeted_ratio()) < fabs(b->GetRatio()-this->_targeted_ratio());
            });
            objVec[0]->IncrementVote();
            cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, -6),6, CV_RGB(0, 255, 255),-1 );
          }
          ...
          if( _vote_higher() )
          {
            ...
            cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, -12),6, CV_RGB(255, 255, 0),-1 );
          }
          ...
          objVec[0]->IncrementVote();
          cv::circle(_output_image, cv::Point(objVec[0]->GetCenter().x,objVec[0]->GetCenter().y) - cv::Point(12, -18),6, CV_RGB(255, 150, 150),-1 );
