...
//        std::stringstream ss;
//        std::cout <<" Could not set the DMA buffer size on "<<config_.GetName()
...
//      std::stringstream ss;
//      ss <<"Conversion from DC1394 frame to cv::Mat failed on "<<config_.GetName()
//                 << " with message: "<< dc1394_error_get_string(error);
//      ROS_ERROR_NAMED(CAM_TAG.c_str() , ss.str().c_str() );
      status_ = Status::ERROR;
      return false;
...
//      ROS_ERROR_NAMED(CAM_TAG.c_str(), "Error in image getting on %s", config_.GetGUID());
      return false;
    }
    return true;
  }
...
//          //ROS_INFO_NAMED(ss.str(), "Setting gain to manual");
      ...
      case Feature::FRAMERATE:
        ...
        std::cout <<"settting to : "<< value
            << " got " << ConvertFramerateToEnum(test) <<std::endl;
        break;
