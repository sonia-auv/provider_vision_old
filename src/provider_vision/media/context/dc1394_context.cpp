...
namespace vision_server {
  ...
  //------------------------------------------------------------------------------
  //
  void DC1394Context::InitContext(const std::vector<CameraConfiguration> &cam_configuration_lists) {
    _context = dc1394_new();
    ...
    for (uint i = 0; i < list->num; i++)
    {
      for( auto const  &cam_config: cam_configuration_lists)
      {
        if( cam_config.GetGUID() == list->ids[i].guid)
        {
          ...
          std::unique_ptr<DC1394Camera> cam( new DC1394Camera ( camera_dc, cam_config ));
          ...
          camera_list_.insert(
              std::make_pair(cam_config.GetName(),
                             std::move( cam ) ) );
        }
      }
    }
  ...
  void DC1394Context::CloseContext() {
    for( auto & active_camera : camera_list_)
    {
  ...
  bool DC1394Context::WatchDogFunc() {
    ...
    for (auto &active_camera : camera_list_) {
      ...
      DC1394Camera &cam = dynamic_cast<DC1394Camera&>(*(active_camera.second.get()));
  ...
  //------------------------------------------------------------------------------
  //
  void DC1394Context::SetFeature(BaseCamera::Feature feat, const std::string &name,
                                 float val) {
    DC1394Camera &cam = GetCameraFromMap(name);
  ...
  //------------------------------------------------------------------------------
  //
  void DC1394Context::GetFeature(BaseCamera::Feature feat, const std::string &name,
                                 float &val) const {
    DC1394Camera &cam = GetCameraFromMap(name);
