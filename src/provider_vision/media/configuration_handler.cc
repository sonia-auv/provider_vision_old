/**
 * \file	configuration_handler.cpp
...
std::vector<CameraConfiguration> ConfigurationHandler::ParseConfiguration() {
  ...
  for (; camera; camera = camera.next_sibling()) {
    ...
    for (auto config_element = camera.first_child(); config_element;
      ...
      if (XML_CAMERA_CAMERA_ID_NODE.compare(config_element.name()) == 0) {
        // ATTRIBUTES
      ...
      } else if (XML_CAMERA_UNDISTORTION_MATRICE_NODE.compare(
                     config_element.name()) == 0) {
