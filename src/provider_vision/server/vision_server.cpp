...
namespace vision_server {
    ...
    bool VisionServer::CallbackExecutionCMD(
        ...
        if (rqst.cmd == rqst.START)
        {
            try
            {
                ...
                Filterchain::Ptr filterchain =
                        filterchain_mgr_.InstanciateFilterchain(rqst.node_name, rqst.filterchain_name);

                detection_task_mgr_.StartDetectionTask(node_handle_, media,
                                                       filterchain, rqst.node_name);

            }
            catch (const std::exception &e)
            {
                std::cout << "Starting execution error: " << e.what() << std::endl;
            }
        ...
        else if (rqst.cmd == rqst.STOP)
        {
            ...
            {
                std::cout << "Closing execution error: " << e.what() << std::endl;
            }
        }
        return true;
    }
