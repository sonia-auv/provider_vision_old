...
namespace vision_server {
    ...
    const std::string FilterchainManager::FILTERCHAIN_MANAGER_TAG = "FILTERCHAIN_MANAGER";
...
//
    Filterchain::Ptr FilterchainManager::InstanciateFilterchain(
        ...
        if (FilterchainExists(filterchainName)) {
            Filterchain::Ptr filterchain = std::make_shared<Filterchain>(filterchainName, executionName);
            _runningFilterchains.push_back(filterchain);
            return filterchain;
        }
        ROS_ERROR_NAMED(FILTERCHAIN_MANAGER_TAG.c_str(), "Could not find the filterchain: %s",
...
//
    bool
    FilterchainManager::CloseFilterchain(
            std::string executionName, std::string filterchainName)
    {
