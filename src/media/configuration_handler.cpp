...
static const std::string XML_CAMERA_UNDISTORTION_MATRICE_NODE = "UndistortionMatrice";
...
ConfigurationHandler::ParseConfiguration()
{
  ...
  for (; camera; camera = camera.next_sibling()) {
    ...
    for (auto attr = camera.first_attribute();
    ...
    // NODES
    for(auto config_element = camera.first_child();
        config_element; config_element = config_element.next_sibling() )
    {
      ...
      if( XML_CAMERA_UNDISTORTION_MATRICE_NODE.compare(config_element.name()) == 0 )
      {
    ...
    const std::map<std::string, CameraConfiguration> &system_config ) const
{
  ...
  for(auto &config : system_config)
  {
    ...
    // ATTRIBUTE
    auto attribute = camera_node.append_attribute(XML_CAMERA_NAME_ATTRIBUTE.c_str());
    attribute.set_value(config.first.c_str());
    attribute = camera_node.append_attribute(XML_CAMERA_GUID_ATTRIBUTE.c_str());
    unsigned long long int guid = static_cast<unsigned long long int>(configuration.GetGUID());
    attribute.set_value(guid);

    auto undistord_matrice_node =
        camera_node.append_child(XML_CAMERA_UNDISTORTION_MATRICE_NODE.c_str());
    undistord_matrice_node.set_value(configuration.GetUndistortionMatricePath().c_str());

    // NODE
    for( const auto &config_element: configuration.GetConfigurations() )
    {
      auto config_element_node = camera_node.append_child(config_element.first.c_str());
