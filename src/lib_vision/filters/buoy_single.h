  ...
  public:
    ...
    : Filter(globalParams),
      ...
      _max_x_difference_for_elimination("Min_x_difference", 50.0f, 0.0f,
                                        ...
                                        _ratio_for_angle_check( "Ratio_for_angle_check", 50, 0, 100, parameters_, "When ratio smaller, will discard if vertical contour"),
                                        _admissible_horizontal_angle("horizontal_max_angle", 50,0,90, parameters_, "When angle smaller, is consider vertical")
    {
      setName("BuoySingle");
    }
    ...
    virtual void execute(cv::Mat &image) {
      if (_enable()) {
        ...
        for (int j = 0; j < contours.size(); j++) {
          ...
          if (_debug_good_contour()) {
            temp.drawFeature(_outputImage, CV_RGB(255, 0, 0), CV_RGB(255, 0, 0), 3);
          }
          ...
          // Angle of contour. If more rectangular, check to make sure it is horizontal
          // not vertical, horizontal is caused by washed out buoy in the sun, while
...
//          std::cout << "New frame " << featuresVect[0].getArea() << " " << featuresVect[1].getArea() << " " << featuresVect[2].getArea() << std::endl;
...
//              std::cout << "i " << featuresVect[i].center << "\t j " << featuresVect[j].center
//                  << "\t IsSameX " << IsSameX(featuresVect[i], featuresVect[j]) << "\t IsHigher "
//                  << IsHigher( featuresVect[i], featuresVect[j] ) << std::endl;
        ...
        if( _detect_red() && featuresVect.size() > 1 )
        {
          ...
          cv::Mat roiA(hsv, cv::Rect(cv::Point(center_a.x-5, (center_a.y)), cv::Size(20,20)));
          cv::Mat roiB(hsv, cv::Rect(cv::Point(center_b.x-5, (center_b.y)), cv::Size(20,20)));
  ...
  private:
    ...
    {
      double x_difference = static_cast<double>(a.center.x ) - static_cast<double>(b.center.x );
    ...
    // Params
    BooleanParameter _enable, _debug_good_contour, _eliminate_same_x_targets, _detect_red;
    StringParameter _color;
    DoubleParameter _min_area, _max_ratio, _min_filled_percent,
    _max_x_difference_for_elimination, _ratio_for_angle_check, _admissible_horizontal_angle;
  };
